    ...
    def inpainting(self, image, mask_b, i): #MaskGIT inference
        ...
        with torch.no_grad():
            ...
            for step in range(self.total_iter):
                ...
                """ 這邊用到了VQGAN_Transformer裡面的inpainting函數 """
            ...
            # 將甜蜜點的圖片儲存到test_results 以便計算FID
            if sweet_spot_result is not None:
                vutils.save_image(sweet_spot_result, os.path.join("./Result/test_results", f"image_{i:03d}.png"), nrow=1)

            vutils.save_image(maska, os.path.join("./Result/mask_scheduling", f"test_{i}.png"), nrow=10)    # 創造出2x10的圖
            vutils.save_image(imga, os.path.join("./Result/imga", f"test_{i}.png"), nrow=7) # 3x7的圖
...
class MaskedImage:
    ...
    def get_mask_latent(self,mask):    
        ...
        # print("max mask_tokens:", mask_tokens.max().item())
        # assert mask_tokens.max().item() < 256, "Index out of range!"  # 添加這行來檢查索引範圍
...
#TODO3 step1-2: modify the path, MVTM parameters
    parser.add_argument('--load-transformer-ckpt-path', type=str, default='transformer_checkpoints/best_ckpt_8(val=0.9).pt', help='load ckpt')
    ...
    # 創建進度條
    for i, (image, mask) in enumerate(tqdm(zip(t.mi_ori, t.mask_ori), total=len(t.mi_ori), desc="Processing images", ncols=120)):
