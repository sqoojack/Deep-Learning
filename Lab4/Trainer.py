...
class VAE_model(nn.Module):
    ...
    """ 繪製圖表 (PSNR per Frame) """
    def plot_psnr_per_frame(self, psnr_frame):  
    ...
    """ 在這邊不會用到make_gif() -> delete """
        
    def train_dataloader(self):
        
        """   進行數據增強 -> 以防止overfitting (沒效, 不要用) """
...
if __name__ == '__main__':
    ...
    parser.add_argument('--kl_anneal_cycle', type=int, default=3, help="")
    parser.add_argument('--kl_anneal_ratio', type=float, default=1, help="")
    
    args = parser.parse_args()
    main(args)

""" use tfr = 0.75, tfr_sde = 6, tfr_d_step = 0.2
    Cyclical: cycle: 3, ratio: 1
    Monotonic: cycle: 1, ratio:0.5
    
    tfr case1: use Full_KL, tfr = 0.75, tfr_sde = 6, tfr_d_step = 0.2
    tfr case2: tfr: 1, tfr_sde = 5, tfr_sde = 0.1
    tfr case3: tfr: 0
